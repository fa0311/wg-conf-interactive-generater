...
export const encodeKey = (value: Uint8Array): string => Buffer.from(value).toString("base64");

export const generateRootKey = (): string => encodeKey(randomBytes(WG_KEY_BYTES));
...
export const parseRootKey = (rootKey: Uint8Array) => {
	...
	const deriveBytes = (label: string): Uint8Array => {
		const derived = hkdfSync("sha256", rootKey, HKDF_SALT, Buffer.from(label, "utf8"), WG_KEY_BYTES);
